...
class Vis():
    ...
    def __init__(self, ques_name, ini_num, file_desti, x=[], y=[],u=[]):
        ...
        # Automatically get all .py filenames (without extension) in current folder
    ...
    def loss_read(self, module_name):
        # The iteration steps of several models do not need to be the same length, only the loss order needs to be the same
    ...
    def loss_vis(self):
        for j in range (len(self.loss_header)-1):
            plt.figure(figsize=(6.6,6)) # Adjust image size
    ...
    # Read the parameters to be trained
    def para_read(self, module_name):
        ...
        # If you want to read parameters, you must have read the loss function together, so there's no need to repeat append here, so you must read the loss function first
        # self.group_name.append(self.module_name)
        # self.module_num+=1
    
    def para_vis(self):
        for j in range (len(self.para_header)-1):
            plt.figure(figsize=(4.4,4)) # Adjust image size
            for i in range (self.module_num):
